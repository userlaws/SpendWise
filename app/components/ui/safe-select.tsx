// This is a safe version of the select component that prevents empty string values
...
SafeSelectScrollUpButton.displayName =
...
SafeSelectScrollDownButton.displayName =
...
// This is the critical component that ensures no empty string values
const SafeSelectItem = React.forwardRef<
  React.ElementRef<typeof SelectPrimitive.Item>,
  React.ComponentPropsWithoutRef<typeof SelectPrimitive.Item>
>(({ className, children, ...props }, ref) => {
  // This is the key fix - we ensure the value is never an empty string
  const safeProps = { ...props };

  if (!safeProps.value || safeProps.value === '') {
    // Create a unique default value
    safeProps.value = `default-${Math.random().toString(36).substring(2, 9)}`;
    console.warn(
      'SafeSelectItem received an empty/missing value, using generated default value'
    );
  }
